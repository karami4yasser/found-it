...
type AddFeedbackParamProps = NativeStackScreenProps<RootStackParamList, 'AddFeedback'>;

export type AddFeedbackProps = {

} & AddFeedbackParamProps;
...
const AddFeedback = (props: AddFeedbackProps) => {
    ...
    const addFeedback = useCallback(async () => {
        try {
            ...
            const response = await AddFeedbackApiCall(accessToken, {
                rating: rating,
                comment: comment,
            }, userId);
            if (response.status === 200) {
                Toaster.show("Feedback Submitted Successfuly.", 1500, true, COLORS.green);
                setLoading(false);
